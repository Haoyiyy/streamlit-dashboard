...
# --- 函數：抓取並解析資料 ---
# 使用 Streamlit 快取資料 10 分鐘 (import requests
import streamlit as st
import pandas as pd
import json

# --- 配置 ---
# 您必須在 Streamlit Cloud 介面設定這個 CWA_API_KEY 變數。
try:
    API_KEY = st.secrets["CWA_API_KEY"]
except KeyError:
    # 如果本地測試，且未設置 secrets.toml，API_KEY 將為 None
    API_KEY = None 
...
def fetch_weather_data(location_name):
    ...
    # 1. 檢查 API Key 是否已正確載入
    if not API_KEY:
        st.error("🚨 錯誤：未找到中央氣象署 API 授權碼。")
        st.info("如果您在 Streamlit Cloud 上部署，請在 Settings > Secrets 中設定 key: CWA_API_KEY")
        st.info("如果您在本地執行，請在 .streamlit/secrets.toml 中設定 key: CWA_API_KEY")
        return None
    ...
    try:
        ...
        if status_code == 401:
            ...
            st.info("請檢查 Streamlit Cloud Secrets 中的金鑰是否正確且有效。")
        else:
            st.error(f"連線錯誤: {e}")
        return None
...
if location:
    ...
    st.markdown("---")
    st.info("💡 數據來源：中央氣象署開放資料平台")ttl=600 秒)，避免頻繁呼叫 API
@st.cache_data(ttl=600) 
def fetch_weather_data(location_name):
    """從中央氣象署 API 抓取 36 小時天氣預報資料，並包含錯誤處理"""
    
    # 1. 檢查 API Key 是否已填入 (修正檢查條件：檢查是否為原始佔位符)
    if API_KEY == "CWA-FF1A0347-64B8-4CBE-8214-580F9D17514D": 
        st.error("🚨 錯誤：請在程式碼中填入您的中央氣象署 API 授權碼！")
        st.info("請檢查 weather_dashboard.py 檔案中 API_KEY 變數的值。")
        return None
    ...
    try:
        ...
        # 2. 發送 API 請求
        res = requests.get(url, timeout=10)
        res.raise_for_status() # 如果狀態碼不是 200，會拋出 HTTPError 異常
        
        data = res.json()
        
        # 3. 檢查 API 回應是否包含有效的地點資料 (處理您之前遇到的 IndexError)
        if "records" not in data or "location" not in data["records"] or not data["records"]["location"]:
             st.warning(f"⚠️ API 回應中找不到 {location_name} 的資料。請檢查 API Key 或地點名稱是否正確。")
             return None
